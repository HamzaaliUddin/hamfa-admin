...
import { BrandsAddEditExistingMedia, BrandsAddEditNewMedia } from './BrandsAddEditMediaView';

const BrandsAddEditMedia = ({ isEdit }: { isEdit?: boolean }) => {
  const {
    watch,
    setValue,
    control,
  ...
  const existingFile = watch('existing_logo');
  const selectedFile = watch('image');
  const imageUrl = watch('image_url');
  const handleRemove = () => {
    setValue('existing_logo', null);
    setValue('image', null);
    setValue('image_url', '');
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {hasUrlPreview ? (
        <BrandsAddEditExistingMedia url={imageUrl} handleClear={handleRemove} />
      ) : /* Priority 2: New file preview */
      newFile || !isEmpty(selectedFile) ? (
        <BrandsAddEditNewMedia file={newFile ? newFile : selectedFile} handleClear={handleRemove} />
      ) : /* Priority 3: Existing image (fallback) */
      isAlreadyAdded ? (
        <BrandsAddEditExistingMedia url={existingFile} handleClear={handleRemove} />
      ) : null}
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="text-muted-foreground px-4 text-center text-xs">
                Upload a brand logo (square image)
              </span>
...
export default BrandsAddEditMedia;
